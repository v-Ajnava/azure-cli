...
class SBSubscriptionCURDScenarioTest(ScenarioTest):
    ...
    def test_sb_namespace(self, resource_group):
        ...
        createsubscriptionresult = self.cmd(
            'servicebus subscription create --resource-group {rg} --namespace-name {namespacename} --topic-name {topicname} --name {subscriptionname} --enable-batched-operations True',
            checks=[self.check('name', self.kwargs['subscriptionname'])]).output

        # Get Create Subscription
        getsubscriptionresult = self.cmd(
            'servicebus subscription get --resource-group {rg} --namespace-name {namespacename} --topic-name {topicname} --name {subscriptionname}',
            checks=[self.check('name', self.kwargs['subscriptionname'])]).output

        # Get list of Subscription
        listsubscriptionresult = self.cmd(
            'servicebus subscription list --resource-group {rg} --namespace-name {namespacename} --topic-name {topicname}').output
