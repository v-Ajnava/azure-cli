...
# AZURE CLI SERVICEBUS - ALIAS GEO DR TEST DEFINITIONS
import json
import time
...
class SBAliasCURDScenarioTest(ScenarioTest):
    ...
    def test_sb_alias(self, resource_group):
        ...
        checknamespaceresult = self.cmd('sb namespace check_name_availability --name {namespacenameprimary}',
        ...
        createnames1paceresult = self.cmd('sb namespace create --resource-group {rg} --name {namespacenameprimary}'
                                          ' --location {loc_south} --tags {tags} --sku-name {sku} --sku-tier {tier}',
                                          checks=[self.check('sku.name', self.kwargs['sku'])]).output

        # Get Created Namespace - Primary
        getnamespace1result = self.cmd('sb namespace get --resource-group {rg} --name {namespacenameprimary}',
                                       checks=[self.check('sku.name', self.kwargs['sku'])]).output

        # Create Namespace - Secondary
        createnamespace2result = self.cmd('sb namespace create --resource-group {rg} --name {namespacenamesecondary}'
                                          ' --location {loc_north} --tags {tags} --sku-name {sku} --sku-tier {tier}',
                                          checks=[self.check('sku.name', self.kwargs['sku'])]).output

        # Get Created Namespace - Secondary
        getnamespace2result = self.cmd('sb namespace get --resource-group {rg} --name {namespacenamesecondary}',
                                       checks=[self.check('sku.name', self.kwargs['sku'])]).output
        ...
        createauthorizationruleresult = self.cmd(
           'sb namespace authorizationrule create --resource-group {rg} --namespace-name {namespacenameprimary}'
           ' --name {authoname}', checks=[self.check('name', self.kwargs['authoname'])]).output
        ...
        getauthorizationruleresult = self.cmd(
            'sb namespace authorizationrule get --resource-group {rg} --namespace-name {namespacenameprimary}'
            ' --name {authoname}', checks=[self.check('name', self.kwargs['authoname'])]).output

        # CheckNameAvailability - Alias

        checkaliasresult = self.cmd('sb alias check_name_availability --resource-group {rg}'
                                    ' --namespace-name {namespacenameprimary} --alias {aliasname}',
                                    checks=[self.check('nameAvailable', True)]).output

        # Create alias
        createalias = self.cmd('sb alias create  --resource-group {rg} --namespace-name {namespacenameprimary}'
                               ' --alias {aliasname} --partner-namespace {id}').output
        ...
        breakpairing = self.cmd(
            'sb alias break_pairing  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}')\
            .output
        ...
        createalias = self.cmd(
            'sb alias create  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}'
            ' --partner-namespace {id}').output
        ...
        while json.loads(getaliasaftercreate)['provisioningState'] != ProvisioningStateDR.succeeded.value:
            time.sleep(30)
            getaliasaftercreate = self.cmd(
                'sb alias get  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}')\
                .output

        # FailOver
        failover = self.cmd(
            'sb alias fail_over  --resource-group {rg} --namespace-name {namespacenamesecondary} --alias {aliasname}')\
            .output
        ...
        while json.loads(getaliasafterfail)['provisioningState'] != ProvisioningStateDR.succeeded.value:
            time.sleep(30)
            getaliasafterfail = self.cmd(
                'sb alias get  --resource-group {rg} --namespace-name {namespacenamesecondary} --alias {aliasname}')\
                .output

        # Delete Alias
        deleteailais = self.cmd(
            'sb alias delete  --resource-group {rg} --namespace-name {namespacenamesecondary} --alias {aliasname}')\
            .output
