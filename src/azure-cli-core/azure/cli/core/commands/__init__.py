...
def _load_command_loader(loader, args, name, prefix):
    ...
    for key, val in module.__dict__.items():
        try:
            if issubclass(val, AzCommandsLoader) and not val == AzCommandsLoader:
                command_loaders[key] = val
        except TypeError:
            pass
    ...
    if command_loaders:
        ...
        logger.debug("Command module '%s' does not have an AzCommandsLoader...", name)
